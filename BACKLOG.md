...
**Last triaged:** 2026-04-14 (added the memory-policy decision, expanded permission-layer persistence hardening, `update.sh` manifest-based change detection, and the `vigil use` default-profile swap as next-session candidates; reordered the list so dependencies appear before items that depend on them).
...
- **`git pre-push` hook tooling + `claude-init-repo` helper.** A hook that requires interactive confirmation before any push, plus a helper that installs the hook into a target repo. Directly addresses the commit-poisoning gap. Larger than a typical "easy" feature. Superseded by `VIGIL_PLAN.md`'s commit-review gate design; removal of this bullet is part of Track 3 of the rebrand.
