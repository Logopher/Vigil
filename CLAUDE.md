...
This repo holds Vigil, my personal default-deny Claude Code configuration, also intended for deployment to friends' machines. ./install.sh copies both profile bundles into `~/.config/vigil/profiles/` (`default/` and `permissive/` as real directories) and additionally renders the active profile into `~/.claude/` (a real directory) where Claude Code reads it. It also installs the shell wrappers, policies, and scripts under `~/.config/vigil/`. Edits here only take effect after re-running the installer — the copy is a deliberate firewall between repo state and live session behavior. Forward direction: the Dashboard codebase (~/code/dashboard/) will move into this repo as a long-lived v2 branch (GENERALIZATION.md "2026-05-13 calibration"; BACKLOG.md for the discrete next-session items); current repo contents are unchanged and will become the Claude Code adapter under the v2 umbrella.

Repo layout:

- `profiles/<name>/` — per-profile bundle: `settings.json`, `settings.local.template.json`, `CLAUDE.md`, `agents/`. Two ship: `default` (strict-by-construction) and `permissive` (lighter floor — only `rm`, `sudo`, `vigil-install-review` denied at the profile layer). Both are copied directly by the installer into `~/.config/vigil/profiles/<name>/`; the active one is additionally rendered into `~/.claude/`.
...
- `install.sh` — copy-based installer; refuses to run if any Vigil-owned destination already exists. Checks specific files/dirs within `~/.claude/` rather than the directory itself, so Claude Code runtime state there does not block reinstallation. Writes the SHA-256 install manifest at `~/.config/vigil/.install-manifest` (via `scripts/install-manifest.py write`) as the final step — `update.sh` reads it to identify user-edited files.
- `update.sh` — refresh-in-place: snapshots manifest entries before uninstall, reconciles divergence post-install by staging the fresh copy as `<name>.new.<ext>` and restoring the user's edit to the live path. Falls back to gap-fill restore for unmanifested files (custom agents, policies, runtime state); installs that predate the manifest skip the snapshot silently and degrade to fresh-install-wins for that one run.

## Architecture

Two layers of configuration, merged by the Claude Code harness at session start:

1. **Profile** (`~/.claude/settings.json` and `~/.claude/settings.local.json`) — sandbox mode, the baseline `deny` list, and hooks wiring. Default profile is plan-mode with a hard `deny` list covering `rm`, `sudo`, non-read-only `git`, network fetchers (`curl`, `wget`), SSH-family tools (`ssh`, `scp`, `rsync`, etc.), language runtimes (`node`, `python`, `python3`, `npx`), a few risky tools (`npm publish`, `docker`, `kubectl`), and credential paths (`~/.ssh/`, `~/.aws/`, etc.). `~/.claude/` is a real directory shared with Claude Code's runtime state (credentials, sessions, history). Both profile bundles are real directories under `~/.config/vigil/profiles/` (`default/` and `permissive/`); `~/.claude/` is populated from the active bundle at install time and at each `vigil set-default` invocation. The permissive profile has a minimal profile-layer deny list (`rm`, `sudo`, `vigil-install-review`). Hook wiring is identical across both profiles.
...
The active profile (what new sessions read) is switched persistently via `vigil set-default <profile>`, which swaps the bundle's `settings.json`, `CLAUDE.md`, `settings.local.template.json`, and `agents/` into `~/.claude/` and records the choice in `~/.config/vigil/active-profile`. Files in `~/.claude/` that the bundle does not name (Claude Code's runtime state) are left untouched. Per-session override is via `CLAUDE_CONFIG_DIR=$HOME/.config/vigil/profiles/<profile>`; with no env var, sessions read from `~/.claude/`.

Hooks are dispatched through a single `vigil-hook` Python binary (sudo-installed at `/usr/local/bin/vigil-hook`). Subcommands wired in both profiles:
...
Hook commands in `settings.json` are bare `vigil-hook <subcommand>` invocations resolved via PATH. The installer no longer substitutes `{{PROFILE_DIR}}` for hook paths. `settings.local.template.json` holds permission-layer Write/Edit/MultiEdit denies for paths the in-process file-editing tools could otherwise reach (`~/.claude/` configuration, `~/.config/vigil/`, shell rc files, `.gitconfig`); these complement sandbox-layer enforcement in `MASTER_DENY_READ`/`MASTER_DENY_WRITE` (`scripts/filter-sandbox-denies.py`), which covers credential reads and subprocess writes to the same enforcement-surface paths.
