...
- `update.sh` — `case "$(uname)" in Darwin|*BSD) … *) …` selects between `cp -n` (BSD) and `cp --update=none` (GNU coreutils ≥ 9.2) for the gap-fill restore step.
...
- **`cp` no-overwrite flag.** GNU coreutils ≥ 9.2 emits a deprecation warning for `cp -n` in favor of `--update=none`; BSD `cp` (macOS, *BSD) does not recognize `--update=none` and only supports `-n`. Both flags have identical "skip if destination exists, exit 0" semantics. Addressed via `uname` branching in `update.sh`'s gap-fill restore step. (The gap-fill restore handles user-added files that the install manifest does not name; user-edited Vigil files are handled separately by the manifest-driven snapshot/reconcile step earlier in `update.sh`, which uses plain `cp` and is not affected by this flag split.)
...
- **`/proc/<pid>/environ` for the wrapper-PID bridge.** `vigil-hook`'s session-context bridge reads claude's environ from procfs to recover `VIGIL_WRAPPER_PID` across the harness's env-strip on hook subprocesses. Procfs is Linux-only; on macOS and *BSD the bridge fails closed and the hook falls back to whatever in-place state exists (currently the legacy singleton; after the rollout, no context — hooks fail open). Per-session isolation between concurrent vigil shells is therefore Linux-only at this layer.
