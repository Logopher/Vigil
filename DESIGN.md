...
`update.sh` has the same "don't silently destroy operator state" principle but, unlike `install.sh`, has a curated overwrite path because it has a hash manifest to drive it. `install.sh` writes `~/.config/vigil/.install-manifest` recording the SHA-256 of every file it places; `update.sh` snapshots the live content of each manifest entry before `uninstall.sh` runs, then post-install distinguishes user-edited files (snapshot hash differs from the recorded baseline) from untouched ones. User-edited files are restored to the live path and the fresh copy is staged adjacent as `<name>.new.<ext>` for manual review. Files the manifest does not name (custom agents, custom policies, runtime state) continue to follow the gap-fill restore path. The first update after an install that predates the manifest feature has nothing to compare against: the snapshot step is skipped silently (the prior install carries no manifest to guard the snapshot conditional), the fresh install wins for that one run, and a stderr note about the missing manifest fires later from the reconcile-setup block. Subsequent updates have a manifest and behave as described.
