...
The sandbox has one deliberate exception: commands listed in `sandbox.excludedCommands` run outside bubblewrap entirely, and their subprocess trees do not inherit its confinement. The default profile uses this narrowly to carve out `git commit *` and `git tag *` so they can reach the host `ssh-agent` for signing — Claude Code's `allowUnixSockets` mechanism is documented but unimplemented on Linux bubblewrap (upstream `anthropics/claude-code#44180`; v2.1.92's seccomp filter additionally blocks AF_UNIX `connect(2)` per `anthropics/claude-code#45072`). The scope is two commands, not a general escape, but their hook subprocesses (`pre-commit`, `prepare-commit-msg`, `commit-msg`, `post-commit`) run with host-level filesystem and network reach. `THREAT_MODEL.md` enumerates the resulting residual.
