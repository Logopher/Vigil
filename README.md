...
`update.sh` defers to `uninstall.sh` to remove only files placed by this repo, moves any surviving state (Claude Code runtime data and user additions like custom agents, hooks, or policies) into a tempdir, runs `install.sh` into the now-empty destinations, then restores the saved state with `cp -r` in no-overwrite mode (`--update=none` on GNU coreutils, `-n` on BSD `cp`) — so for any path that appears in both the saved state and the fresh install, the fresh install wins. On clean exit the tempdir is removed; on failure it is preserved and its path is printed.

**Local edits to installed files are preserved.** `install.sh` writes a SHA-256 manifest at `~/.config/vigil/.install-manifest` recording the as-installed content of every file it places. Before `update.sh` runs `uninstall.sh`, it snapshots the live content of each manifest entry. After the fresh install, any file whose snapshot hash differs from the recorded baseline is treated as user-edited: the fresh copy is staged adjacent as `<name>.new.<ext>` (e.g. `CLAUDE.new.md`, `dev.new.json`) and the user's edit is restored into the live path. The summary at the end of `update.sh` lists every preserved file with its staged sibling so you can review and merge manually. To accept the upstream version, replace the live file with the `.new.<ext>` copy; to keep your edit, delete the `.new.<ext>` file — the live path already holds your version. Either way re-run `python3 ~/.config/vigil/scripts/install-manifest.py write` afterward so the next update doesn't flag the same divergence. Files the installer never wrote (custom agents, custom policies, etc.) continue to follow the gap-fill restore path above.

Two corner cases to be aware of. First, if you re-run `filter-sandbox-denies.py` yourself between updates (e.g. after installing a tool whose credential path you want denied), the recorded `settings.json` hash no longer matches the post-mutation content and the next update flags that file as user-edited. Re-running the manifest writer rebaselines `settings.json` — but it also rebaselines every other tracked file in the same call, silently discarding the divergence record for any other edit you have not yet reviewed. If you have a pending `.new.<ext>` you intend to merge, finish that first; if you want only `settings.json` rebaselined without touching the rest, the cheapest path is to do nothing (live with the divergence flag on the next update) rather than rebaseline everything. Second, the first `update.sh` after upgrading to this feature has no manifest in the prior install's state and silently skips the snapshot for that one run, so user edits to Vigil files are clobbered (no `.new.<ext>` produced); subsequent updates work as described.
