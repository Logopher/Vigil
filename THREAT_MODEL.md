...
The scope also assumes Claude Code is launched through the session wrappers in `vigil-aliases.sh`. Launches via the VS Code extension or the Claude Code desktop app receive the active profile's deny list, hooks, and sandbox configuration, but do not receive session logging, the environment scrub, or per-session policy selection. Mitigations that depend on the posture layer — the audit trail and the env-scrub layer described below — do not apply to desktop-app or extension launches. Under the permissive profile the profile-layer deny list is only `rm`, `sudo`, and `vigil-install-review`; the heavier deny baseline is carried by the policy overlays, which IDE and desktop-app launches do not receive. Users on permissive should treat those launches as having the `vigil-yolo` deny list (plan mode is still in effect, since both profiles set `defaultMode: "plan"`) and rely on terminal wrappers for sessions where the deny list matters.
...
**`excludedCommands` carve-out.** Commands matched by `sandbox.excludedCommands` run outside the sandbox entirely — neither they nor their subprocesses inherit bubblewrap. Both shipped profiles carve out `git commit *`, `git tag *`, `git -C * commit *`, `git -C * tag *`, `git verify-commit *`, and `git verify-tag *` so they can reach the host `ssh-agent` for signing and signature verification; the `allowUnixSockets` path is unimplemented on Linux bubblewrap (tracked upstream as `anthropics/claude-code#44180`; see `BACKLOG.md` → *Concluded investigations*). The `commit` and `tag` variants fire git hook subprocesses — `pre-commit`, `prepare-commit-msg`, `commit-msg`, `post-commit` — that therefore execute with host-level filesystem and network access, not sandbox-confined access. The `verify-commit` and `verify-tag` variants do not fire git hooks and carry no hook subprocess risk. Hook *file installation* remains sandbox-blocked (`denyWrite` on `.git/hooks/`) against subprocess writes, but a hook installed through another channel (notably the in-process Write tool, if not denied at the permission layer) runs outside bubblewrap the next time a `git commit` or `git tag` variant fires.
...
The shell wrappers always apply a policy, and the policies are engineered to carry the full deny baseline themselves rather than relying on the profile to supply it — the `strict` and `dev` policies each contain the full destructive/network/runtime deny set. As a result, `vigil-dev` under permissive has the same deny baseline as `vigil-dev` under default. The bullets below characterize the wrapper combinations rather than (profile, policy) pairs because, under the shipped policies, only the policy varies in practice. The profile difference becomes visible only when no policy is in play — see the last bullet.

- **`vigil` / `vigil-strict`:** equivalent to the default profile baseline under either profile. Plan mode forces deliberate review of every action. Safe for any context.
- **`vigil-dev`:** safe for *trusted project code* where you would be comfortable letting the operator auto-accept edits. Not safe if the project contains executable content from untrusted sources (e.g., a directory of downloaded scripts). Commit poisoning is the primary residual risk.
- **`vigil-yolo`:** safe only for throwaway work on throwaway data. The two remaining denies (`rm`, `sudo`) prevent the worst catastrophes but nothing else.
- **Bare `claude` (no wrapper, no `--settings`):** posture depends entirely on the active profile. Under default, approximately `vigil-strict` minus session logging and env scrubbing — plan mode plus the full deny list. Under permissive, the same deny list as `vigil-yolo` — only `rm`, `sudo`, and `vigil-install-review` — but still in plan mode (not `bypassPermissions` the way `vigil-yolo` would be). IDE and desktop-app launches fall into this category.
...
- **Commit-signing `excludedCommands` carve-out (test 4):** *Verified on 2026-04-15.* Three commits (`b768a97`, `d8fbd1a`, `840f4bd`) made from inside a default-profile session via bare `git commit` — with `commit.gpgsign=true`, `gpg.format=ssh`, `user.signingkey=~/.ssh/github` — each landed with an SSH signature embedded in the commit object (`gpgsig -----BEGIN SSH SIGNATURE-----` block visible via `git cat-file -p`). The in-sandbox agent socket at `/tmp/claude/vigil-signing.sock` has been removed, so the signatures can only have been produced by `git commit` reaching the host-side `ssh-agent` directly — confirming the `excludedCommands` carve-out runs signing outside bubblewrap as designed.
- **In-process tools bypass sandbox `denyRead` and `denyWrite` (test 5):** *Verified on 2026-05-13* (bubblewrap 0.9.0). Under the `yolo` policy, against a non-credential canary path (`~/vigil-canary/`) added to both sandbox `denyRead` and `denyWrite` but with *no* permission-layer `Read(...)`/`Write(...)` deny — so the only layer that could have intercepted the in-process call was the sandbox itself. A credential-shaped path would have triggered Claude's model-level refusal (the same effect that leaves test 3 inconclusive), so a benign canary was chosen to isolate the sandbox question cleanly. Results: the Read tool returned the host file's true contents while subprocess `cat`/`ls` against the same path saw an empty tmpfs overlay; the Write tool created a file on the host (verified post-session from outside the sandbox) while subprocess writes against the same path landed in a per-bwrap-instance tmpfs that did not persist beyond the originating Bash invocation. Confirms the architectural model documented above under *Mitigations by layer → Sandbox*: bubblewrap's filesystem masking applies only to child processes; in-process tools (Read/Write/Edit) share the parent's filesystem view. The permission-layer `deny` list is therefore the only layer that can protect a path from in-process tools.
...
- The `git -C * commit *`, `git -C * tag *`, `git verify-commit *`, and `git verify-tag *` entries in `excludedCommands` use mid-string glob patterns (not pure prefixes). Test 4 verified only the bare `git commit *` prefix form. Whether the harness evaluates `excludedCommands` entries as full globs or prefix-only matches is unverified empirically. `tests/semantics.py` documents the expected fnmatch-style semantics as an explicit assumption, but tests Python's own `fnmatch` — not the harness. A harness that uses prefix-only matching would pass the test suite silently while the mid-string patterns fail to match at runtime. Live session verification is required to close this gap.
