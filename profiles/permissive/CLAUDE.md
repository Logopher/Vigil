...
The Vigil profile denies a fixed set of Bash patterns at the permission layer. Attempting one prompts the user; reflexive retries waste their attention.

- **Destructive / privileged**: `rm`, `sudo`, `vigil-install-review` — ask the user.

All other command denies (git mutations, network fetchers, runtimes, credential paths, etc.) come from the active policy overlay, not this profile. The exact set depends on which `vigil` entry point was used to start the session.
...
Specialist agents ship with this profile in `agents/`. Projects may define additional specialist reviewers in their own `.claude/agents/`.

| Agent | File | When to use |
|---|---|---|
| `architect` | `agents/architect.md` | New features, cross-component changes, any decision affecting structure |
| `code-reviewer` | `agents/code-reviewer.md` | After implementation, before every commit |
